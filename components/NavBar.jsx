  ...
  return (
    <nav className='flex-between w-full mb-16 pt-3'>
        <Link href='/' className='flex gap-2 flex-center'>
            <Image src='/assets/images/logo.svg' alt='PHelper Logo' width={30} height={30} className='object-contain'/>
            <p className='logo_text'>PHelper</p>
        </Link>
        {/* { Desktop Navigation } */}
            ...
            {session?.user ? (
                ...
                <>
                    {providers && Object.values(providers).map((provider) => (
                        <button type='buttons' key={provider.name} onClick={() => signIn(provider.id)} className='black_btn'>
                            Sign In
                        </button>
                    ))}
                </>
